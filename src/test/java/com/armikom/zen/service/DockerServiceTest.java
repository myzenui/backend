...
@SpringBootTest
@TestPropertySource(properties = {"logging.level.com.armikom.zen.service=DEBUG"})
class DockerServiceTest {
    
    private DockerService dockerService;
    private static final String TEST_IMAGE_TAG = "europe-west1-docker.pkg.dev/myzenui/image/zen-backend:latest";
    ...
    void testPushDockerImageWithValidTag() {
        ...
        assumeTrue(dockerService.isDockerAvailable(), "Docker is not available on this system");
    ...
    void testIsDockerAvailable() {
        ...
        assertNotNull(isAvailable, "isDockerAvailable should return a boolean value");
    }
    ...
    void testSpecificImageTagFormat() {
        String expectedTag = "europe-west1-docker.pkg.dev/myzenui/image/zen-backend:latest";
        assertEquals(TEST_IMAGE_TAG, expectedTag, "Test image tag should match the specified format");
        
        // Verify the tag has the expected components
        assertTrue(TEST_IMAGE_TAG.contains("europe-west1-docker.pkg.dev"), "Should contain GCP registry URL");
        assertTrue(TEST_IMAGE_TAG.contains("myzenui"), "Should contain project name");
        assertTrue(TEST_IMAGE_TAG.contains("zen-backend"), "Should contain image name");
        assertTrue(TEST_IMAGE_TAG.endsWith(":latest"), "Should end with :latest tag");
    }
    ...
    void testDockerClientInitializationFailure() {
        ...
        assertFalse(testService.isDockerAvailable(), "Docker should not be available without initialization");
